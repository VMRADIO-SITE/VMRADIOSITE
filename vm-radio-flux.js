/* VM RADIO — compatibilité moteur/flux unique */
(function(){
"use strict";
...
function track(song,meta){if(!song||typeof song!=="object")return null;const title=first(song.title,song.name);if(!title)return null;const playlist=String(meta?.playlist||"").toLowerCase();return{id:String(first(song.id,song.track_id,title)),title:titleCaseFirst(title),artist:String(first(song.artist,song.artist_name)||DEFAULT_ARTIST).trim(),cover:String(first(song.art,song.cover,song.cover_url,song.artwork,song.artwork_url)||""),time:first(meta?.played_at,meta?.started_at,meta?.time),type:meta?.is_request===true?"request":playlist.includes("jingle")?"jingle":"music"}}
async function get(){const r=await fetch(ENGINE+"?_="+Date.now(),{cache:"no-store",credentials:"omit"});if(!r.ok)throw Error("VM RADIO API "+r.status);return r.json()}
function clock(v){if(!v)return"--:--";const d=typeof v==="number"?new Date(v*1000):new Date(v);return Number.isNaN(d.getTime())?"--:--":d.toLocaleTimeString("fr-FR",{timeZone:"Europe/Paris",hour:"2-digit",minute:"2-digit"})}
function text(sel,v){document.querySelectorAll(sel).forEach(e=>e.textContent=String(v??""))}
function image(sel,v){document.querySelectorAll(sel).forEach(e=>{if(e.tagName==="IMG"&&v)e.src=v})}
function enforceStream(){document.querySelectorAll("audio").forEach(a=>{let s="";try{s=a.currentSrc||a.src||""}catch{}if(s===STREAM)return;const play=!a.paused&&!a.ended;try{a.src=STREAM;a.setAttribute("src",STREAM);a.load();if(play)a.play().catch(()=>{})}catch{}})}
